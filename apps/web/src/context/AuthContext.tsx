...
export interface AuthUser { id:string;name:string;email:string;role:string;companyId:string;portal:'admin'|'pos';branchId?:string;warehouseId?:string;branchName?:string;branchCode?:string; }
...
export function AuthProvider({children}:{children:ReactNode}){
  ...
  const login=async(email:string,password:string,portal:'admin'|'pos')=>{
    ...
    const obj:AuthUser={id:u.id,email:u.email,role:u.role,companyId:u.companyId,name:u.name||u.email.split('@')[0],portal:u.portal||portal,branchId:u.branchId,warehouseId:u.warehouseId,branchName:u.branchName,branchCode:u.branchCode};

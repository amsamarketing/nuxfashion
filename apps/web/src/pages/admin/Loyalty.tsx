...
const PROMOS=[['Ramadan 2026 Sale','All categories 20% off · All branches','g'],['GOLD10 Coupon','10% off Gold+ · All channels','g'],['Buy 2 Get 1 Free','Accessories · Riyadh Mall','g'],['Flash Friday Shoes','Shoes 30% off Fri 4–8 PM','a'],['New Season Welcome','First purchase 15% off','g'],['VIP Early Access','Platinum members preview','g']];
export default function Loyalty() {
  return (
    <div>
      <div style={{ display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:10 }}>
        <div><div style={{ fontSize:14,fontWeight:600 }}>Loyalty & promotions</div><div style={{ fontSize:11,color:'var(--text-secondary)' }}>Tiers · Promotions · Coupons · Gift cards</div></div>
      ...
      <div style={{ display:'grid',gridTemplateColumns:'1fr 1fr',gap:8 }}>
        <div className="card">
          ...
          {[['Bronze','0–999 pts','1 pt per SAR 5'],['Silver','1,000–4,999 pts','1 pt per SAR 4'],['Gold','5,000–19,999 pts','1 pt per SAR 3 + 5% birthday bonus'],['Platinum','20,000+ pts','1 pt per SAR 2 + free delivery + VIP']].map(([t,r,d])=>(
            <div key={t} style={{ display:'flex',alignItems:'flex-start',gap:8,padding:'7px 0',borderBottom:'0.5px solid var(--border)' }}>
              <div style={{ flex:1 }}><div style={{ fontWeight:600 }}>{t} <span style={{ fontWeight:400,color:'var(--text-muted)',fontSize:10 }}>{r}</span></div><div style={{ fontSize:10,color:'var(--text-secondary)',marginTop:2 }}>{d}</div></div>

  ...
  return (
    <div>
      <div style={{ display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:10 }}>
        <div><div style={{ fontSize:14,fontWeight:600 }}>Inventory</div><div style={{ fontSize:11,color:'var(--text-secondary)' }}>{data?.length||0} active SKUs</div></div>
        <div style={{ display:'flex',gap:5 }}>
          <button className="bt"><i className="ti ti-arrows-exchange" /> Transfer</button>
          <button className="bt"><i className="ti ti-adjustments" /> Adjust</button>
      ...
      <div className="card" style={{ padding:0,overflow:'hidden' }}>
        <div className="tr th" style={{ gridTemplateColumns:'1fr 60px 70px 80px 70px' }}>
          {['Product / SKU','Qty','Reorder at','Location','Status'].map(h=><span key={h}>{h}</span>)}
        </div>
        {data?.map((i:any)=>{
          ...
          return (
            <div key={i.id} className="tr" style={{ gridTemplateColumns:'1fr 60px 70px 80px 70px',background:low?'var(--bg-danger)':'' }}>
              ...
              <span style={{ fontWeight:700,color:low?'var(--text-danger)':'' }}>{i.quantity}</span>
              <span style={{ color:'var(--text-secondary)' }}>{i.reorder_point}</span>
              <span style={{ color:'var(--text-secondary)' }}>{i.warehouse_name}</span>
              <span><span className={'bx '+(low?'r':'g')}>{low?'Low':'In stock'}</span></span>
            </div>
          );
        })}
      </div>

      ...
      <div style={{ width: 'min(560px,100%)', background: 'var(--cd)', borderRadius: 16, overflow: 'hidden' }} onClick={e => e.stopPropagation()}>
        ...
        <div style={{ padding: 24, display: 'grid', gap: 14, maxHeight: '70vh', overflowY: 'auto' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            <input type="checkbox" id="whactive" checked={form.is_active} onChange={e => F('is_active', e.target.checked)} style={{ width: 16, height: 16 }} />
            <label htmlFor="whactive" style={{ fontSize: 13, cursor: 'pointer' }}>Active warehouse</label>
          </div>
        </div>
...
export default function Warehouses() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['warehouses'],
    queryFn: async () => { const r = await api.get('/inventory/warehouses'); return r.data; },
  });
  const { data: whStock } = useQuery({
    queryKey: ['wh-stock', sel?.id],
    queryFn: async () => { const r = await api.get(`/inventory/warehouses/${sel.id}/stock`); return r.data; },
    enabled: !!sel?.id,
  });
          ...
          {isLoading ? <div style={{ color: 'var(--mu)' }}>Loading...</div> : warehouses.length === 0 ? <div style={{ color: 'var(--mu)' }}>No warehouses yet.</div> : warehouses.map((w: any) => (
            <div key={w.id} className="nx-card" style={{ cursor: 'pointer', border: sel?.id === w.id ? '2px solid var(--ac)' : '2px solid transparent' }} onClick={() => { setSel(w); setSelTab('info'); }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 14 }}>
                <div style={{ width: 44, height: 44, borderRadius: 12, background: 'var(--acg)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><i className="ti ti-building-warehouse" style={{ fontSize: 22, color: 'var(--ac)' }} /></div>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 4, alignItems: 'flex-end' }}>
                  <span className={`nx-badge ${w.is_active ? 'active' : 'inactive'}`}>{w.is_active ? 'Active' : 'Inactive'}</span>
                  <span className={`nx-badge ${TYPE_COLOR[w.type] || 'teal'}`} style={{ fontSize: 10 }}>{w.type || 'main'}</span>
                </div>
              </div>
        ...
        {sel && (
          <div style={{ background: 'var(--cd)', borderRadius: 14, border: '1px solid var(--bd)', overflow: 'hidden', position: 'sticky', top: 80 }}>
            <div style={{ padding: '16px 20px', borderBottom: '1px solid var(--bd)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <div><div style={{ fontWeight: 700, fontSize: 16 }}>{sel.name}</div><div style={{ fontSize: 12, color: 'var(--mu)' }}>{sel.code || '-'}</div></div>
            ...
            <div style={{ padding: 20, maxHeight: 500, overflowY: 'auto' }}>
              ...
              {selTab === 'stock' && (
                <div>
                  {stock.length === 0 ? (
                    <div style={{ textAlign: 'center', padding: '32px 0', color: 'var(--mu)' }}><i className="ti ti-package" style={{ fontSize: 32, display: 'block', marginBottom: 8 }} /><div>No stock data</div></div>

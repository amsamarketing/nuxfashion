  ...
  return (
    <div>
      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:10 }}>
        <div><div style={{ fontSize:14, fontWeight:600 }}>HR & payroll</div>
          <div style={{ fontSize:11, color:'var(--text-secondary)' }}>{employees?.length||0} employees · {depts?.length||0} departments</div></div>
      ...
      <div style={{ display:'flex', gap:5, marginBottom:10 }}>
        {['Employees','Attendance','Payroll','Commissions','GOSI','Leave','Schedules'].map((t,i)=><button key={t} className={'snb'+(i===0?' on':'')}>{t}</button>)}
      </div>
      <StatRow stats={[
      ...
      <div className="card" style={{ padding:0, overflow:'hidden' }}>
        <div className="tr th" style={{ gridTemplateColumns:'1fr 100px 70px 90px 80px 70px' }}>
        ...
        {employees?.map((e:any) => {
          ...
          return (
            <div key={e.id} className="tr" style={{ gridTemplateColumns:'1fr 100px 70px 90px 80px 70px' }}>
              ...
              <span style={{ color:'var(--text-secondary)' }}>SAR {gosi.toLocaleString()}</span>

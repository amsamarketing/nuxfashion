...
const orderVat=(o:any)=>{
  const stored=Number(o?.tax_amount||o?.vat_amount||0);if(stored>0)return stored;
  const total=Number(o?.total||0),paid=Number(o?.amount_paid||o?.paid_amount||0);
  return paid>total+0.01?paid-total:orderGross(o)*15/115;
};
...
export default function Orders(){
  ...
  const exportCsv=()=>{
    const esc=(v:any)=>`"${String(v??'').replace(/"/g,'""')}"`;const rows=[['Invoice','Date','Customer','Phone','Cashier','Items','Subtotal','Discount','VAT 15%','Total','Paid','Returned','Payment','Status'],...orders.map(o=>[o.order_number,o.created_at,o.customer_name,o.customer_phone,o.cashier_name,o.item_count,o.subtotal,o.discount_amount,orderVat(o),orderGross(o),o.paid_amount,o.returned_amount,o.payment_method,o.status])];

  ...
  return (
    <div>
      <div style={{ display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:10 }}>
        <div><div style={{ fontSize:14,fontWeight:600 }}>Orders</div><div style={{ fontSize:11,color:'var(--text-secondary)' }}>All channels</div></div>
      ...
      <div className="card" style={{ padding:0,overflow:'hidden' }}>
        <div className="tr th" style={{ gridTemplateColumns:'90px 1fr 90px 90px 70px' }}>
          {['Order #','Customer','Payment','Total','Status'].map(h=><span key={h}>{h}</span>)}
        </div>
        {data?.map((o:any)=>(
          <div key={o.id} className="tr" style={{ gridTemplateColumns:'90px 1fr 90px 90px 70px' }}>
            <span style={{ fontWeight:600 }}>#{o.order_number}</span>
            <span>{o.customer_name||'Walk-in'}</span>
            <span style={{ color:'var(--text-secondary)',textTransform:'capitalize' }}>{o.payment_method||'—'}</span>

  ...
  return (
    <div>
      <div style={{ display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:10 }}>
        <div><div style={{ fontSize:14,fontWeight:600 }}>Purchasing</div><div style={{ fontSize:11,color:'var(--text-secondary)' }}>{data?.length||0} purchase orders</div></div>
        <div style={{ display:'flex',gap:5 }}>
          <button className="bt"><i className="ti ti-users" /> Suppliers ({suppliers?.length||0})</button>
          <button className="bt bt-p"><i className="ti ti-plus" /> New PO</button>
        </div>
      </div>
      <StatRow stats={[{label:'Total POs',value:data?.length||0},{label:'Suppliers',value:suppliers?.length||0},{label:'Total Value',value:'SAR '+((data||[]).reduce((s:number,p:any)=>s+parseFloat(p.total||0),0)).toLocaleString()}]} />
      <div className="card" style={{ padding:0,overflow:'hidden' }}>
        ...
        {data?.map((po:any)=>(
          <div key={po.id} className="tr" style={{ gridTemplateColumns:'90px 1fr 100px 100px 70px' }}>
            ...
            <span style={{ fontWeight:600 }}>SAR {parseFloat(po.total||0).toFixed(2)}</span>
            <span><span className={'bx '+(sc[po.status]||'n')}>{po.status}</span></span>
          </div>
        ))}
      </div>
    </div>

...
const getPrice = (p: any, v: any, field: string) => {
  const val = v?.[field] ?? p?.[field] ?? p?.price ?? 0;
...
function exportCSV(products: any[]) {
  ...
  products.forEach((p:any) => {
    ...
    rows.push([p.name, p.name_ar||'', v?.sku||p.sku||'', v?.barcode||p.barcode||'',
      ...
      p.category_name||'', p.brand_name||'', p.is_active?'Active':'Inactive']);
  });
...
export default function Products() {
  ...
  inventory.forEach((i:any) => {
    const key = i.variant_id || i.product_id || i.sku;
    if (key) stockMap[key] = (stockMap[key]||0) + (i.quantity||0);
  });

  const getStock = (p: any) => {
    const v = p.variants?.[0];
    return stockMap[v?.id] ?? stockMap[p.id] ?? stockMap[v?.sku] ?? null;
  };
  ...
  const stockMut = useMutation({
    ...
    onSuccess: () => { toast('Stock added!', 'success'); qc.invalidateQueries({queryKey:['inventory']}); setStockItem(null); setStockQty(''); },
    onError: (e:any) => {
      // Try alternative endpoint
      const p = stockItem;
      const v = p.variants?.[0];
      api.patch('/inventory/' + (v?.id || p.id), { quantity: parseInt(stockQty), reason: 'Initial stock entry' })
        .then(() => { toast('Stock added!', 'success'); qc.invalidateQueries({queryKey:['inventory']}); setStockItem(null); setStockQty(''); })
        .catch(() => toast(getErr(e), 'error'));
    },
  });
  ...
  const openEdit = (p:any) => {
    const v = p.variants?.[0];
    setForm({
      name: p.name, name_ar: p.name_ar||'', sku: v?.sku||p.sku||'', barcode: v?.barcode||p.barcode||'',
      selling_price: String(getPrice(p,v,'selling_price')||''),
      cost_price: String(getPrice(p,v,'cost_price')||''),
  ...
  const handleCSV = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const nameIdx = idx(['name']); const skuIdx = idx(['sku']);
    const priceIdx = idx(['selling','price']); const costIdx = idx(['cost']);
    let ok=0, fail=0;
    for (let i=1; i<lines.length; i++) {
      const cols = lines[i].split(',').map(c=>c.replace(/^"|"$/g,'').trim());
      const name = nameIdx>=0 ? cols[nameIdx] : ''; if (!name) continue;
      try {
        await api.post('/catalog/products', {
          name, sku: skuIdx>=0?cols[skuIdx]:undefined,
    ...
    <div>
      ...
      {isLoading ? <div className="d-flex justify-content-center py-5"><div className="spinner-border text-primary"/></div> : (
        <div className="card" style={{padding:0,overflow:'hidden'}}>
          <div className="tr th" style={{gridTemplateColumns:'1fr 90px 90px 90px 80px 75px 70px 140px'}}>
          ...
          {filtered.map((p:any)=>{
            ...
            return (
              <div key={p.id} className="tr" style={{gridTemplateColumns:'1fr 90px 90px 90px 80px 75px 70px 140px', opacity:p.is_active?1:.55}}>
                <span>
                ...
                <span>
                  {stock===null
                    ? <button className="bt" style={{fontSize:9,padding:'3px 6px',color:'var(--text-warning-custom)'}} onClick={()=>{setStockItem(p);setStockQty('');}}><i className="ti ti-plus"/> Stock</button>
                    : <span className={'bx '+(stock<=0?'r':stock<=5?'a':'g')} style={{fontSize:9}}>{stock<=0?'Out':stock+' pcs'}</span>}
                </span>
                <span style={{display:'flex',gap:4}}>
                  <button className="bt" style={{fontSize:10,padding:'4px 7px'}} title="Edit" onClick={()=>openEdit(p)}>
      ...
      {stockItem && (
        <Modal title={'Add stock — ' + stockItem.name} onClose={()=>setStockItem(null)}>
          <div style={{padding:'10px 12px',background:'var(--surface-1)',borderRadius:'var(--radius)',marginBottom:16,fontSize:12,color:'var(--text-secondary)'}}>
            This product has no inventory record yet. Set the opening stock quantity.
          </div>
          <Field label="Opening stock quantity" required>
            <Inp type="number" value={stockQty} onChange={v=>setStockQty(v)} placeholder="e.g. 50"/>
          </Field>
          <div className="d-flex gap-2 justify-content-end">
            <button className="bt" onClick={()=>setStockItem(null)}>Cancel</button>
            <SaveBtn label="Add to inventory" loading={stockMut.isPending} disabled={!stockQty} onClick={()=>stockMut.mutate()}/>
          </div>
        </Modal>
      )}

    ...
    <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,.55)',zIndex:1000,display:'flex',alignItems:'center',justifyContent:'center',padding:16}} onClick={onClose}>
      <div style={{width:'min(600px,100%)',background:'var(--cd)',borderRadius:16,overflow:'hidden',display:'flex',flexDirection:'column',maxHeight:'90vh'}} onClick={e=>e.stopPropagation()}>
        ...
        <div style={{padding:20,overflowY:'auto',flex:1,display:'grid',gap:14}}>
          ...
          <div style={{padding:12,border:'1px solid #c4b5fd',background:'rgba(124,58,237,.05)',borderRadius:10,display:'grid',gap:9}}><div><b style={{fontSize:13}}><i className="ti ti-sparkles"/> AI Lifestyle Studio</b><div style={{fontSize:10,color:'var(--mu)'}}>Uses the primary image and preserves product color, design and logo.</div></div><div style={{display:'grid',gridTemplateColumns:'1fr 1fr 80px',gap:7}}><select className="nx-select" value={ai.style} onChange={e=>setAi(x=>({...x,style:e.target.value}))}><option value="studio">Luxury Studio</option><option value="mall">Saudi Mall</option><option value="riyadh">Riyadh Street</option><option value="ramadan">Ramadan Evening</option><option value="resort">Desert Resort</option></select><select className="nx-select" value={ai.gender} onChange={e=>setAi(x=>({...x,gender:e.target.value}))}><optgroup label="Adults"><option value="female">Female Model</option><option value="modest_female">Modest Female Model</option><option value="hijab">Hijab Model</option><option value="male">Male Model</option><option value="plus_female">Plus-size Female</option><option value="plus_male">Plus-size Male</option><option value="couple">Couple</option><option value="family">Family</option></optgroup><optgroup label="Kids"><option value="boy">Boy</option><option value="girl">Girl</option><option value="baby">Baby</option></optgroup><optgroup label="Product only"><option value="mannequin">Mannequin</option><option value="ghost">Ghost Mannequin</option><option value="flatlay">Flat Lay</option><option value="hands">Hand Model</option><option value="none">No Model</option></optgroup></select><select className="nx-select" value={ai.count} onChange={e=>setAi(x=>({...x,count:e.target.value}))}><option value="1">1 image</option><option value="2">2 images</option><option value="3">3 images</option></select></div><input className="nx-input" value={ai.prompt} onChange={e=>setAi(x=>({...x,prompt:e.target.value}))} placeholder="Optional direction: modest abaya styling, warm evening light..."/><button className="btn-nx primary" disabled={!prod?.id||!images.length||generate.isPending} onClick={()=>generate.mutate()}>{generate.isPending?'Creating lifestyle image...':'Generate AI Lifestyle Image'}</button>{!prod?.id&&<small style={{color:'var(--mu)'}}>Save this new product first, then edit it to use AI Lifestyle Studio.</small>}</div>

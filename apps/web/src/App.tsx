...
const ADMIN_NAV = [
  {id:'ad-dash',l:'Dashboard',i:'ti-layout-dashboard'},
  {id:'ad-orders',l:'Orders',i:'ti-shopping-cart'},
  {id:'ad-inv',l:'Inventory',i:'ti-package'},
  {id:'ad-prod',l:'Products',i:'ti-tag'},
  {id:'ad-purch',l:'Purchasing',i:'ti-truck'},
  {id:'ad-acct',l:'Accounting',i:'ti-report-money'},
  {id:'ad-zatca',l:'ZATCA invoices',i:'ti-file-check'},
  {id:'ad-crm',l:'Customers',i:'ti-users'},
  {id:'ad-loyal',l:'Loyalty & promos',i:'ti-star'},
  {id:'ad-hr',l:'HR & payroll',i:'ti-id'},
  {id:'ad-rep',l:'Reports',i:'ti-chart-bar'},
  {id:'ad-set',l:'Settings',i:'ti-settings'},
];
...
function App() {
  ...
  return (
    <div style={{ minHeight:'100vh', background:'var(--bg-primary)', padding:16, fontFamily:'-apple-system,BlinkMacSystemFont,"Segoe UI",system-ui,sans-serif' }}>
      <div style={{ maxWidth:1100, margin:'0 auto' }}>
        ...
        <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'9px 16px', background:'var(--surface-2)', borderBottom:'0.5px solid var(--border)', borderRadius:'14px 14px 0 0' }}>
          {mode==='pos' ? (
            <div style={{ display:'flex', alignItems:'center', gap:10 }}>
              <span style={{ fontSize:14, fontWeight:600 }}>NuxFashion</span>
              <span style={{ color:'var(--text-secondary)' }}>Riyadh Mall · Terminal 1</span>
              <span className="bx n"><i className="ti ti-clock" style={{ fontSize:11 }} /> Morning shift</span>
            </div>
          ) : (
            <div style={{ display:'flex', alignItems:'center', gap:10 }}>
              <span style={{ fontSize:14, fontWeight:600 }}>NuxFashion ERP</span>
              <span className="bx n">NuxFashion KSA · Multi-branch</span>
            </div>
          )}
        ...
        <div style={{ display:'flex', background:'var(--surface-1)', borderBottom:'0.5px solid var(--border)' }}>
          {[['pos',<><i className="ti ti-device-desktop" style={{ fontSize:14 }} /> POS terminal</>],['admin',<><i className="ti ti-layout-dashboard" style={{ fontSize:14 }} /> Admin portal</>]].map(([m, label]) => (
            <button key={m as string} onClick={() => switchMode(m as 'pos'|'admin')}
              style={{ flex:1, padding:9, fontSize:13, fontWeight:500, cursor:'pointer', border:'none', background:'transparent', color:mode===m?'var(--text-accent)':'var(--text-secondary)', borderBottom:mode===m?'2px solid var(--fill-accent)':'2px solid transparent', transition:'all 0.15s' }}>
              {label}
            </button>
          ))}
        </div>
        ...
        {/* Main content */}
        <div style={{ display:'grid', gridTemplateColumns: mode==='admin' ? '155px 1fr' : '1fr' }}>
          {mode==='admin' && (
            <div style={{ padding:'8px 6px', borderRight:'0.5px solid var(--border)', background:'var(--surface-2)', minHeight:500 }}>
          ...
          <div style={{ padding:'12px 14px', background:'var(--surface-0)', minHeight:500, overflow:'auto' }}>
            <Screen />
          </div>
        </div>

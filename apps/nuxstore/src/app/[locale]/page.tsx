...
export default async function HomePage() {
  ...
  const renderSection=(section:any,index:number)=>{
    ...
    if(section.type==='hero'){const all=config.banners||[];const chosen=Array.isArray(section.banner_ids)?section.banner_ids.map((id:string)=>all.find((b:any)=>b.id===id)).filter(Boolean):all;return chosen.length?<div key={key} className="max-w-[1640px] mx-auto px-2 pt-2 md:px-4 md:pt-4"><div className="aspect-[4/5] min-h-0 overflow-hidden rounded-[10px] sm:aspect-[16/9] md:aspect-[2.67/1] md:min-h-[420px] md:rounded-[14px]"><HeroBanner locale={locale} banners={chosen} autoplaySeconds={speed||5}/></div></div>:null}

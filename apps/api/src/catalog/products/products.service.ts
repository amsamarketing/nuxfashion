...
export class ProductsService implements OnModuleInit {
  ...
  async generateLifestyle(productId:string,companyId:string,body:any){
    ...
    const presentations:any={female:'an adult female fashion model',modest_female:'an adult female model in elegant modest Saudi styling',hijab:'an adult female fashion model wearing a coordinated hijab with modest styling',male:'an adult male fashion model',plus_female:'an adult plus-size female fashion model',plus_male:'an adult plus-size male fashion model',boy:'a boy child model with age-appropriate catalog styling',girl:'a girl child model with age-appropriate catalog styling',baby:'a baby model with safe age-appropriate catalog styling',couple:'an adult male and female fashion-model couple',family:'a Saudi family in tasteful age-appropriate commercial styling',mannequin:'a premium retail mannequin',ghost:'an invisible ghost-mannequin product presentation showing the garment shape',flatlay:'a premium top-down flat-lay arrangement',hands:'a professional hand model displaying the product',none:'a premium product-only retail display without a person'};
    const presentation=presentations[body?.gender]||presentations.none;
    const prompt=`Create a photorealistic vertical ecommerce lifestyle photograph featuring the exact product from the reference image in ${styles[body?.style]||styles.studio}. Present it using ${presentation}. Preserve the product design, color, fabric, logo and proportions exactly. Full product clearly visible, natural commercial lighting, culturally appropriate Saudi market styling, no text, no watermark. ${String(body?.prompt||'').slice(0,500)}`;

...
export class AuthService{
  ...
  async login(loginDto:LoginDto,ipAddress:string,portal:'admin'|'pos'='admin'){
    ...
    const posRole=this.isPosRole(user.role_name);if(portal==='admin'&&posRole)throw new ForbiddenException('This account can only sign in from the Branch POS login page.');if(portal==='pos'&&!posRole)throw new ForbiddenException('Use a branch POS user account. Administrator accounts open the admin panel.');
    ...
    const payload={sub:user.id,email:user.email,role:user.role_name,companyId:user.company_id,portal,branchId:branch?.branch_id,warehouseId:branch?.warehouse_id};
    ...
    return{accessToken,refreshToken,user:{id:user.id,name:user.name,email:user.email,role:user.role_name,companyId:user.company_id,portal,branchId:branch?.branch_id,warehouseId:branch?.warehouse_id,branchName:branch?.branch_name,branchCode:branch?.branch_code}};
  }

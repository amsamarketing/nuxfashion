...
export class StorefrontService {
  ...
  async checkout(dto:StoreCheckoutDto){
    ...
    const companyId=await this.tenantId();
    const shippingFee=Number(this.config.get('STOREFRONT_SHIPPING_FEE')||25);
    const freeFrom=Number(this.config.get('STOREFRONT_FREE_SHIPPING_FROM')||300);
    return this.db.transaction(async client=>{
      ...
      return {order_number:orderNumber,status:'confirmed',payment_method:dto.payment_method,subtotal,shipping,vat:tax,total,customer:{name:dto.customer_name,phone:dto.phone},estimated_delivery:'2–5 business days'};
    });
  }
}
